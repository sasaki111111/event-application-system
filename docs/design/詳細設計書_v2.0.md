...
sequenceDiagram
  ...
  loop 各申込について
    alt statusが「キャンセル待ち」かつ区分あり
      ...
      S->>AR: countByEvent_IdAndTicketTypeIsNullAndStatusAndAppliedAtLessThan(eventId, "キャンセル待ち", appliedAt)
      S->>S: waitlistRank = 件数 + 1
    else それ以外
...
| 対象DTO | 制約 |
|---|---|
| `EventUpsertRequest` | name必須・100文字以内／startAt必須・未来日時／place必須・100文字以内／capacity必須・1以上／applicationDeadline必須／description1000文字以内／organizerName・imageUrl・extraQuestionは任意（各文字数上限あり）／ticketTypesの各要素はname必須1〜50文字・capacity必須1以上 |
...
| | `countByEvent_IdAndTicketTypeIsNullAndStatusAndAppliedAtLessThan(eventId, status, appliedAt)` | キャンセル待ちの順位計算（区分の無いイベント） |
