...
フェーズA-4 成果物 ／ v1.3（2026-09-16。更新: 機能追加（ソフトデリート）によりevents.deleted_atを追加）
...
| deleted_at | DATETIME | NULL | - | - | **機能追加（ソフトデリート）**。NULL=有効、日時あり=削除済み。物理削除はせず、管理者の「削除済みイベント」画面から復元できる |
| created_at | DATETIME | NOT NULL | CURRENT_TIMESTAMP | - | 作成日時（監査カラム） |
| updated_at | DATETIME | NOT NULL | CURRENT_TIMESTAMP（更新時に自動更新） | - | 更新日時（監査カラム） |
...
| `applications.event_id` | `events.id` | **CASCADE**（要確認・推奨値） | イベント削除可否チェック（要件定義§8）は「`受付済`の申込が無ければ削除可」。`キャンセル済`の申込行は残り得るため、物理削除を成立させるには CASCADE が必要。**機能追加（ソフトデリート）によりAPI経由の削除（API-08）は`deleted_at`を立てるのみで物理削除はもう発生しないが、DB制約自体は保険として残す** |
...
> `deleted_at`は上記と異なり派生値ではなく実カラム（§2.2）。一覧・詳細系API（API-01/02/09）は`deleted_at IS NULL`を、削除済み一覧API（機能追加）は`deleted_at IS NOT NULL`を条件に加えて絞り込む。
...
| `events.deleted_at`（機能追加：ソフトデリート） | 要件定義書§7「削除済みイベント」画面と一致 |
...
| イベント削除（ソフトデリート）・復元（機能追加） | `events.deleted_at` |
| 権限チェック | `users.role` |
...
| 削除済みイベント画面（機能追加） | 復元対象イベント（ボタン） | **選択 → ID型の主キー**（復元対象の特定） | `events.id`（更新対象は`deleted_at`） |
...
| 削除済み一覧API（機能追加） | id, name, startAt, place, capacity, deletedAt | `events`の同名列（`deletedAt`は`deleted_at`） | `deleted_at IS NOT NULL`で絞り込み |
| 復元API（機能追加） | id（パス変数） | `events.id` | `deleted_at`をNULLに戻す |
