...
| `applications.ticket_type_id` | `ticket_types.id` | RESTRICT | 区分の変更（全置換）はアプリ側で「対象の区分に受付済・キャンセル待ちの申込が無いこと」を事前チェックしてから削除する。本制約は、それをすり抜けるケース（キャンセル済の申込が区分を参照したまま残っている場合）を防ぐ最終的な安全網 |
...
`deleted_at`／`organizer_name`／`image_url`／`extra_question`は上記と異なり派生値ではなく実カラム（§2.2）。一覧・詳細系APIは`deleted_at IS NULL`を、削除済み一覧APIは`deleted_at IS NOT NULL`を条件に加えて絞り込む。
...
| 業務ロジック16件（要件定義書§8） | すべてこのテーブル構成で判定可能（下表） |
...
| 区分変更時の申込チェック | `applications`（`event_id`, `status`の存在確認。`ticket_type_id`が無い申込も対象）、および`ticket_type_id`のFK制約 |
...
| SC-04 登録・編集フォーム | イベント名／開催日時／場所／定員／申込締切／説明／主催者名／画像URL／アンケート文言／区分（複数） | 直接対応 | `events`の同名列、`ticket_types`（複数行） |
...
| API | 項目 | 対応するDBカラム | 備考 |
|---|---|---|---|
| API-01/02 | id, name, startAt, place, capacity, applicationDeadline, description, organizerName, imageUrl | 各テーブルの同名列 | 直接対応 |
...
| API-06/07 | name〜imageUrl・extraQuestion、ticketTypes[] | `events`の同名列、`ticket_types` | |
...
各テーブルのID（主キー・外部キー）について、どこで発行され、どこで参照・利用され、
なぜそのIDが必要なのかを1つの表にまとめる（§4の外部キー制約、§7のトレーサビリティ表を
横断して整理したもの）。

| テーブル.カラム | 種別 | 発行元（どこで作られるか） | 利用先（画面・API） | 必要性（無いと何が起きるか） |
|---|---|---|---|---|
| `users.id` | PK | 初期データ投入、または会員登録（API-11）実行時にDBが自動採番 | 全画面・全APIの`X-User-Id`ヘッダによる本人特定。`applications`／`favorites`／`event_comments`の`user_id`として申込者・登録者・投稿者を特定 | 氏名・メールだけでは同姓同名や表記揺れで本人を一意に特定できず、認証ヘッダにも他テーブルとの関連付けにも使えない |
| `events.id` | PK | イベント登録（API-06）実行時にDBが自動採番 | SC-02詳細画面のルートパラメータ（`/events/:id`）。`applications`／`favorites`／`ticket_types`／`event_comments`の`event_id`として対象イベントを特定 | イベント名は重複しうるため、名前だけでは「どのイベントへの申込／お気に入り／区分／コメントか」を一意に確定できない |
| `applications.id` | PK | 申込（API-03）実行時にDBが自動採番 | マイページの「キャンセル」ボタン（`DELETE /api/applications/{id}`）、当日受付の「チェックイン」ボタン（`PUT /api/applications/{id}/check-in`）で対象申込を指定 | キャンセル後の再申込等で同一ユーザー・同一イベントの申込行が複数存在しうるため、idが無いと「取り消す／チェックインする対象がどの申込か」を特定できない |
| `applications.user_id` | FK→users | 申込（API-03）実行時、`X-User-Id`ヘッダの値をサーバーが設定（リクエストボディには含まれない） | マイページ「自分の申込一覧」の絞り込み、本人以外によるキャンセル防止 | 誰の申込かを記録できないと、マイページの表示も本人確認によるキャンセル制御もできない |
| `applications.event_id` | FK→events | 申込（API-03）実行時、リクエストの`eventId`から設定 | イベント単位の受付済数集計・定員超過チェック、イベント削除可否チェック | どのイベントへの申込かが分からないと、定員判定・削除可否チェック・実績集計が一切できない |
| `applications.ticket_type_id` | FK→ticket_types（任意） | 申込（API-03）実行時、リクエストの`ticketTypeId`から設定（区分の無いイベントはNULL） | 区分単位の定員超過チェック・受付済数集計、区分削除時の安全チェック（§4） | これが無いと区分（一般枠／会員枠等）ごとの定員管理ができず、v2.0で追加した区分機能自体が成立しない |
| `favorites.user_id`／`event_id` | FK→users／events | お気に入り登録（API-15）実行時 | マイページ「お気に入り」タブの一覧表示、重複登録防止のUNIQUE制約（§2.4） | 誰がどのイベントをお気に入りにしたかを一意に記録できないと、一覧表示も重複防止もできない |
| `ticket_types.event_id` | FK→events | イベント登録・編集（API-06/07）実行時 | イベント詳細・申込画面での区分選択肢の表示、編集フォームでの既存区分一覧取得 | どのイベントの区分かが分からないと、申込画面での選択肢表示や区分ごとの定員合計チェックができない |
| `event_comments.event_id`／`user_id` | FK→events／users | コメント投稿（API-21）実行時 | イベント詳細のコメント一覧表示、削除可否チェック（投稿者本人か、§4） | 投稿者・対象イベントが分からないと、一覧表示にも削除権限チェック（E10）にも使えない |
